  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      final record = HabitRecord(
        ...
        isBackfilled: false,
        createdAt: now,
        source: RecordSource.fromList, // 从习惯列表打卡
        planId: null, // 习惯列表打卡无关联计划
      );
...
/// 显示计划打卡对话框的辅助函数（次日计划打卡）
/// 返回打卡记录 ID
Future<String?> showPlanCheckInDialog(
  ...
  return showCupertinoDialog<String>(
    context: context,
    barrierDismissible: true,
    builder: (context) => PlanCheckInDialog(habit: habit, plan: plan),
  );
}

/// 计划打卡对话框 - 专门用于次日计划打卡
class PlanCheckInDialog extends ConsumerStatefulWidget {
  final Habit habit;
  final DailyPlan plan;

  const PlanCheckInDialog({
    super.key,
    required this.habit,
    required this.plan,
  });
  ...
  @override
  ConsumerState<PlanCheckInDialog> createState() => _PlanCheckInDialogState();
}

class _PlanCheckInDialogState extends ConsumerState<PlanCheckInDialog>
    with SingleTickerProviderStateMixin {
  int? _selectedQuality; // 1-5 星评分
  final _notesController = TextEditingController();
  bool _isSubmitting = false;
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      final record = HabitRecord(
        ...
        isBackfilled: false,
        createdAt: now,
        source: RecordSource.fromPlan, // 从次日计划打卡
        planId: widget.plan.id, // 关联计划 ID
      );
